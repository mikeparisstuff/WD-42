...
struct WebServer {
    ...
    request_queue_arc: Arc<~[HTTP_Request]>,
    stream_map_arc: Arc<HashMap<~str, Option<std::io::net::tcp::TcpStream>>>,
    cache_arc: Arc<HashMap<~str, ~[u8]>>,

    visitor_count_arc: Arc<int>,
...
impl WebServer {
    fn new(ip: &str, port: uint, www_dir: &str) -> WebServer {
        ...
        WebServer {
            ...
            stream_map_arc: Arc::new(HashMap::new()),

            visitor_count_arc: Arc::new(0),
            cache_arc: Arc::new(HashMap::new()),
    ...
    fn listen(&mut self) {
        ...
        spawn(proc() {
            ...
            for stream in acceptor.incoming() {
                let (queue_port, queue_chan) = channel();
                ...
                let (counter_port, counter_chan) = channel();
    ...
    fn respond_with_static_file(stream: Option<std::io::net::tcp::TcpStream>, path: &Path, cache_arc: Arc<HashMap<~str, ~[u8]>>) {
    ...
    fn enqueue_static_file_request(stream: Option<std::io::net::tcp::TcpStream>, path_obj: &Path, stream_map_arc: Arc<HashMap<~str, Option<std::io::net::tcp::TcpStream>>>, req_queue_arc: Arc<~[HTTP_Request]>, notify_chan: SharedChan<()>) {
        // Save stream in hashmap for later response.
        let mut stream = stream;
        let peer_name = WebServer::get_peer_name(&mut stream);
        let (stream_port, stream_chan) = channel();
    ...
    fn dequeue_static_file_request(&mut self) {
        ...
        let (request_port, request_chan) = channel();
        let (semaphore_port, semaphore_chan) = channel();
        let (cache_port, cache_chan) = channel();
